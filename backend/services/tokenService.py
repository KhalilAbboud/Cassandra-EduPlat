import mmh3  # murmur3 — doit être en haut du fichier
...
def explain_partition(cluster_name: str, keyspace: str, table: str, partition_key: str):
    ...
    # FIX: protéger contre un ring vide (cluster pas encore prêt)
    if not all_tokens:
        raise Exception("Token ring is empty — cluster may still be initializing. Try again in a few seconds.")

    # 4. Trouver le nœud responsable (premier token >= hash, sinon wrap around)
    responsible = next((t for t in all_tokens if t["token"] >= hash_value), all_tokens[0])
