...
def create_node(payload: NodeCreate) -> NodeResponse:
    ...
    container, host_port = create_cassandra_node(
        node_name=payload.name, 
        cluster_name=payload.cluster_name,
        initial_token=payload.initial_token
    )
    ...
    # Retry jusqu'à avoir les tokens et l'IP
    tokens = []
    ip = ""
    for attempt in range(12):  # max 60 secondes
        time.sleep(5)
        container.reload()
        networks = container.attrs["NetworkSettings"]["Networks"]
        ip = networks.get(network_name, {}).get("IPAddress", "")
        tokens = get_node_tokens(container)
        print(f"Attempt {attempt+1}: ip={ip}, tokens={len(tokens)}")
        if ip and tokens:
            break
