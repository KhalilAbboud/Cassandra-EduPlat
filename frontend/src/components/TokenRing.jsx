...
export default function TokenRing({
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", userSelect: "none", fontFamily: "'JetBrains Mono','Fira Code',monospace" }}>
      ...
      >
        {/* Ring */}
        <circle cx={CX} cy={CY} r={RING_R + 3}  fill="none" stroke="rgba(124,106,247,0.08)" strokeWidth={14} />
        <circle cx={CX} cy={CY} r={RING_R}       fill="none" stroke="rgba(255,255,255,0.10)" strokeWidth={1.5} strokeDasharray="6 4" />
        ...
        {sorted.length >= 2 && sorted.map((node, i) => {
          ...
          return (
            <path key={`arc-${node.id}`}
              ...
              strokeWidth={highlightSet.has(node.id) ? 4 : 2}
            />
          );
        })}
        ...
        {/* Palette node */}
        {!palDragging && (
          <g onMouseDown={onPaletteMD} style={{ cursor: "grab" }}>
            ...
            <text x={PALETTE.x} y={PALETTE.y + NODE_R + 14}
              textAnchor="middle" fontSize={8} fill="rgba(255,255,255,0.3)">drag to ring</text>
          </g>
        )}
          ...
          <div style={{
            ...
            {data.length === 0
              ...
              : <>
                  ...
                  {data.length > 9 && <div style={{ color: "rgba(255,255,255,0.2)", fontSize: 9, marginTop: 2 }}>+{data.length - 9} more</div>}
                </>
            }
