    ...
    <div style={card}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {loading && <span style={{ fontSize: 9, color: "#8AA8C0" }}>polling…</span>}
      ...
      {/* Educational blurb */}
      <div style={{ fontSize: 10, color: "#5A7A96", lineHeight: 1.6, marginBottom: 10, padding: "8px 10px", background: "rgba(245,158,11,0.05)", borderLeft: `2px solid ${AMBER}44`, borderRadius: 4 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 6, marginBottom: 10, flexWrap: "wrap" }}>
        <div style={{ flex: 1, padding: "8px 10px", background: "rgba(245,158,11,0.06)", borderRadius: 6, border: `1px solid ${AMBER}22` }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: hints.length > 0 ? AMBER : "#8AA8C0", ...mono }}>{hints.length}</div>
        </div>
        <div style={{ flex: 1, padding: "8px 10px", background: "rgba(239,68,68,0.06)", borderRadius: 6, border: `1px solid ${RED}22` }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: downNodes.length > 0 ? RED : "#8AA8C0", ...mono }}>{downNodes.length}</div>
        </div>
      </div>
      ...
      {rawStats && (
        <div style={{ marginTop: 8 }}>
          <button onClick={() => setExpanded(v => !v)}
            style={{ fontSize: 9, background: "transparent", border: "none", color: "#8AA8C0", cursor: "pointer", padding: 0 }}>
            {expanded ? "▾" : "▸"} raw nodetool tpstats
          </button>
          {expanded && (
            <pre style={{ marginTop: 4, fontSize: 8, color: "#8AA8C0", background: "rgba(0,0,0,0.3)", padding: 8, borderRadius: 5, overflowX: "auto", maxHeight: 140 }}>
              {rawStats}
            </pre>
          )}
        </div>
      ...
      {/* Activity log */}
      {log.length > 0 && (
        <div style={{ marginTop: 10, maxHeight: 110, overflowY: "auto", borderTop: "1px solid rgba(26,43,60,0.08)", paddingTop: 8 }}>
          {log.map((entry, i) => (
